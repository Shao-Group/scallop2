...
int bundle_bridge::min_three(int x, int y, int z) { return std::min(std::min(x, y), z); }
...
double bundle_bridge::get_Jaccard(string s, string t)
{
	int kmer_length = 10;
...
bool bundle_bridge::are_strings_similar(string s, string t)
{
	...
	if(match_count >= floor(t.size()/15.0)) return true;
	...
	for(int k = 0; k < fragments.size(); k++)
	{
		...
		if(soft_clip_side == 1  && left_boundary_match == 1)
		{
			...
			for(int j=0;j<regions.size();j++)
			{
				...
				hash = to_string(fr.h1->pos) + "|" + new_s;

				int32_t pos1 = rc.rpos-effective_len+1;
				int32_t pos2 = rc.rpos;

				string region_seq = get_fasta_seq(pos1,pos2);

				int edit_match = 0;

				assert(region_seq.size() == new_s.size());

				//bool is_similar = are_strings_similar(new_s,region_seq);
		...
		else if(soft_clip_side == 2 && right_boundary_match == 1)
		{
			...
			for(int j=0;j<regions.size();j++)
			{
				...
				hash = to_string(fr.h2->rpos) + "|" + new_s;
...
int bundle_bridge::join_circ_fragment_pair(pair<fragment,fragment> &fr_pair, int ex1, int ex2, int left_boundary_flag, int right_boundary_flag)
{
	...
	if(fr2.is_compatible == 1)
	{
		...
		int32_t len = 0;
		for(int i=0;i<fr2.h2->cigar_vector.size();i++)
		{
			if(fr2.h2->cigar_vector[i].first == 'M')
			{
				len += fr2.h2->cigar_vector[i].second;
			}
		}
		circ.supple_len = len;
	...
	else if(fr2.is_compatible == 2)
	{
		...
		//calculate supple_len from cigar
		int32_t len = 0;
		for(int i=0;i<fr2.h1->cigar_vector.size();i++)
		{
			if(fr2.h1->cigar_vector[i].first == 'M')
			{
				len += fr2.h1->cigar_vector[i].second;
			}
		}
		circ.supple_len = len;
