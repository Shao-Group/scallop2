...
int scallop3::assemble()
{
	...
	//printf("in scallop3.cc: trsts.size = %d, non full length trsts.size = %d\n", trsts.size(), non_full_trsts.size());
...
bool scallop3::resolve_smallest_edges(double max_ratio)
{
	int se = -1;
	int root = -1;
...
bool scallop3::resolve_negligible_edges(bool extend, double max_ratio)
{
	bool flag = false;
	//for(set<int>::iterator it = nonzeroset.begin(); it != nonzeroset.end(); it++)
	//for(int i = 1; i < gr.num_vertices() - 1; i++)
...
bool scallop3::resolve_splittable_vertex(int type, int degree, double max_ratio)
{
	int root = -1;
	double ratio = max_ratio;
...
bool scallop3::resolve_unsplittable_vertex(int type, int degree, double max_ratio)
{
	int root = -1;
	MPID pe2w;
	double ratio = max_ratio;
...
bool scallop3::resolve_hyper_edge(int fsize)
{
	edge_iterator it1, it2;
	PEEI pei;
	vector<int> v1, v2;
...
bool scallop3::resolve_trivial_vertex(int type, double jump_ratio)
{
	int root = -1;
...
bool scallop3::resolve_trivial_vertex_fast(double jump_ratio)
{
	bool flag = false;
	//for(set<int>::iterator it = nonzeroset.begin(); it != nonzeroset.end(); it++)
	vector<int> vv(nonzeroset.begin(), nonzeroset.end());
...
bool scallop3::resolve_single_trivial_vertex_fast(int i, double jump_ratio)
{
...
int scallop3::summarize_vertices()
{
	for(set<int>::iterator it = nonzeroset.begin(); it != nonzeroset.end(); it++)
	{
		int i = (*it);
...
int scallop3::decompose_vertex_extend(int root, MPID &pe2w)
{
...
int scallop3::decompose_vertex_replace(int root, MPID &pe2w)
{
	// reassign weights
	MID md;
	for(MPID::iterator it = pe2w.begin(); it != pe2w.end(); it++)
...
int scallop3::classify_trivial_vertex(int x, bool fast)
{
...
int scallop3::exchange_sink(int old_sink, int new_sink)
{
	VE ve;
	edge_iterator it1, it2;
	PEEI pei;
...
int scallop3::compute_smallest_edge(int x, double &ratio)
{
	int e = -1;
	ratio = DBL_MAX;
	edge_iterator it1, it2;
